...
1. You should give the `#legend-gradient` element a linear gradient that transitions between all the colors from `--color0` to `--color5`. Express the color stops as percentages, using only whole numbers, and make the transition between colors a hard line.
...
You should make the transition from one color to the following color a hard line for your `#legend-gradient`. Remember to use your `--color#` variables, as well as express your color stops in percentages, using only whole numbers.

```js
const legendGradient = new __helpers.CSSHelp(document).getStyle('#legend-gradient');
assert.exists(legendGradient);
const matched = legendGradient.backgroundImage.match(/var\(\s*--color[0-5]\s*\)\s+\d+%/g);
