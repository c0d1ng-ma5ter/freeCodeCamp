...
Now, create a `cardInfo` array inside of your `CardList` component. Declare it using `const`, since you won't need to change it later.

# --hints--

Your should create a `cardInfo` array inside your `CardList` component.

```js
assert.match(__helpers.removeJSComments(code), /const\s*cardInfo\s*=\s*\[.*\]/)
